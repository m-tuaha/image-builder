...
def generate_multi_image_kontext(
    ...
    Uses manual API calls for consistency with other generation functions.
    """
    ...
    try:
        # Step 1: Upload images to Replicate's file storage to get URLs
        uploaded_urls = []
        
        for i, f in enumerate(image_files[:4]):  # Limit to 4 images
            try:
            ...
            filename = getattr(f, 'name', f'image_{i}.png')
            content_type = getattr(f, 'type', 'image/png')
            
            # Upload file using multipart form data (NOT JSON)
            files = {
                "file": (filename, file_data, content_type)
            }
            
            # Upload to Replicate files API (remove Content-Type header for multipart)
            ...
            uploaded_urls.append(upload_data["urls"]["get"])
        
        # Step 2: Create prediction with the uploaded image URLs
        payload = {
            "input": {
                "prompt": prompt.strip(),
        ...
        # Use JSON headers for the prediction request
        json_headers = {
            "Authorization": f"Bearer {token}",
            "Content-Type": "application/json",
            "Prefer": "wait"
        ...
        resp = requests.post(api_endpoint, headers=json_headers, json=payload)
        resp.raise_for_status()
        prediction = resp.json()
        prediction_id = prediction["id"]
        
        # Step 3: Poll for completion (same pattern as your other functions)
        max_wait_time = 300  # 5 minutes
        start_time = time.time()
        ...
        if st.button("🎨 Generate", key="generate_img_btn", use_container_width=True):
            with st.spinner("🎨 Generating your image..."):
                try:
                    ...
                    # Replace your existing "Combine Images" generation logic with this properly indented version:
                        ...
                        # Generate image using multi-image kontext
                        img_bytes = generate_multi_image_kontext(
                            ...
                            model_slug="flux-kontext-apps/multi-image-list"  # or try "multi-image-kontext-max"
                        )
