...
with tab2:
    ...
    mode = st.selectbox("Mode", ["Create", "Inspire", "Revamp"], key="img_mode")
    ...
    if mode == "Create":
        ...
        if st.session_state.chained_image and st.session_state.edit_mode == mode:
            ...
            uploaded = st.file_uploader(
                "Upload an image" + (" to copy style from" if mode=="Inspire" else " to edit"),
                type=["png","jpg","jpeg"],
                key="input_image_file"
            )
            if uploaded:
        ...
        prompt = st.text_input("Enter your prompt", key="img_prompt")

    # Reset & Refine / Generate buttons
    col1, col2 = st.columns([1, 1])
    with col1:
        if st.button("🔄 Reset All", key="reset_all", use_container_width=True):
            for k in ["image_raw_prompt","refined_prompt","chained_image","edit_mode","img_prompt","img_mode"]:
        ...
        if st.button("🎨 Generate", key="generate_img_btn", use_container_width=True):
            with st.spinner("🎨 Generating your image..."):
                try:
                    if mode == "Create":
                        prompt_to_send = (
                            st.session_state.refined_prompt.strip()
                            or st.session_state.image_raw_prompt.strip()
                        )
                        ...
                        img_bytes = generate_kontext_max(prompt, uri)
                    ...
                    # chained‐edit only for the two new modes
                    if mode in ("Inspire", "Revamp") and st.button("✏️ Edit This Image", key="edit_img_btn", use_container_width=True):
