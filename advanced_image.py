...
def generate_multi_image_kontext(
    ...
    try:
        # Step 1: Upload images to get temporary URLs
        uploaded_urls = []
        ...
        for i, f in enumerate(image_files[:4]):  # Limit to 4 images
            ...
            files = {
                "file": (
                    getattr(f, 'name', f'image_{i}.png'),  # filename
                    file_data,  # file data
                    getattr(f, 'type', 'image/png')  # mime type
                )
            }
        ...
        # Step 2: Create prediction with uploaded image URLs
        payload = {
            "input": {
                "prompt": prompt.strip(),
        ...
        resp = requests.post(api_endpoint, headers={**headers, "Content-Type": "application/json"}, json=payload)
        resp.raise_for_status()
        prediction = resp.json()
        prediction_id = prediction["id"]
