...
with tab2:
    ...
    # ----- INSPIRE (single image to copy style) -----
        ...
        prompt = st.text_input("Enter your prompt", key="img_prompt_inspire")

    # ----- COMBINE IMAGES (multi-image kontext) -----
    else:
        st.caption("Upload up to 4 images. The model will combine/transform them per your prompt.")
        # If user is chaining from a previous output, show it and allow up to 3 more uploads
        ...
        # Collect up to 4 total inputs (prefilled + uploaded)
        input_uris = []
        input_count = 0

        # Helper to push bytes->data URI
        def _bytes_to_data_uri(b: bytes, mime: str = "image/png"):
            b64 = base64.b64encode(b).decode()
            return f"data:{mime};base64,{b64}"

        if prefilled:
            input_uris.append(_bytes_to_data_uri(prefilled, "image/png"))
            input_count += 1
        ...
        if multi_files:
            for f in multi_files:
                if input_count >= 4:
                    break
                b = f.read()
                input_uris.append(_bytes_to_data_uri(b, f.type or "image/png"))
                input_count += 1
    ...
    with col2:
        if st.button("🎨 Generate", key="generate_img_btn", use_container_width=True):
            with st.spinner("🎨 Generating your image..."):
                try:
                    if mode == "Create":
                        prompt_to_send = (st.session_state.get("refined_prompt","").strip()
                        ...
                        img_bytes = generate_kontext_max(st.session_state["img_prompt_inspire"].strip(), uri)
...
if multi_files:
    files_for_upload.extend(multi_files[: max(0, 4 - len(files_for_upload))])

img_bytes = generate_multi_image_kontext(
    prompt=st.session_state["img_prompt_combine"].strip(),
    image_files=files_for_upload,  # <— match helper signature
