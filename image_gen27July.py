...
with col3:
    if st.button("🔄 Reset Form"):
        for key in ['vertical', 'product', 'theme', 'style', 'promo_text', 'color_palette', 'audience', 'show_preview', 'prompt_preview', 'include_text']:
    ...
    if not product or product.strip() == "":
        ...
        with st.spinner("🎨 Generating your visual..."):
            try:
                ...
                # Prepare API parameters for DALL-E 3
                api_params = {
                    "model": "dall-e-3",
                    "prompt": final_prompt,
                    "size": "1024x1024",
                ...
                st.image(img_bytes, caption=f"Generated Image (1024×1024)", use_column_width=True)
                
                # Download button
                st.download_button(
                    label="📥 Download Image",
                    data=img_bytes,
                    file_name=f"campaign_{product.lower().replace(' ', '_')}_{style.lower().replace(' ', '_')}.png",
                    mime="image/png",
                    type="primary"
                )
                ...
                if "API key" in str(e):
                    ...
                    st.info("💡 Please verify that your OpenAI API has access to DALL-E 3.")

# Footer
st.markdown("---")
st.markdown("<div style='text-align: center; color: #666; font-size: 0.9rem;'>Powered by DALL-E 3 | Built with Streamlit</div>", unsafe_allow_html=True)
